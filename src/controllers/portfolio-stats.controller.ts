...
import { computePositions, computeTotalRealizedPL } from '../utils/portfolio-calc';
...
export const getPortfolioStats = async (
  ...
  try {
    ...
    // 2. Compute positions and realized_pl
    const positions = computePositions(tradeList);
    const realized_pl = computeTotalRealizedPL(tradeList);
    ...
    // 4. Compute total_value, total_cost, unrealized_pl
    let total_value = 0;
    let total_cost = 0;
    ...
    // 5. Query dividends table: dividend_ytd (current year), dividend_mtd (current month)
    const now = new Date();
