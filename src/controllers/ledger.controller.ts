...
export const getLedgers = async (
  ...
  res: Response<ApiResponse<LedgerWithRole[]>>
): Promise<void> => {
  try {
    const userId = req.user!.id;

    const { data: ledgerUsers, error: luError } = await supabaseAdmin
      .from('ledger_users')
      .select('ledger_id, role')
      .eq('user_id', userId);

    if (luError) {
      throw new AppError('Failed to fetch ledgers', 500);
    }
    ...
    const ledgersWithRole: LedgerWithRole[] = (ledgers || []).map((ledger) => ({
      ...ledger,
      role: roleMap.get(ledger.id) as 'owner' | 'member',
    }));
...
export const createLedger = async (
  ...
  try {
    ...
    if (error || !ledger) {
      throw new AppError('Failed to create ledger', 500);
    }

    const { error: luError } = await supabaseAdmin.from('ledger_users').insert({
      ledger_id: ledger.id,
      user_id: userId,
      role: 'owner',
      created_by: userId,
    ...
    if (luError) {
      await supabaseAdmin.from('ledgers').delete().eq('id', ledger.id);
      throw new AppError('Failed to create ledger membership', 500);
    }

    res.status(201).json({
      success: true,
      data: { ...ledger, role: 'owner' as const },
    });
  } catch (err) {
    if (err instanceof AppError) throw err;
...
export const updateLedger = async (
  req: AuthenticatedRequest,
  res: Response<ApiResponse<Ledger>>
): Promise<void> => {
  try {
    const userId = req.user!.id;
    const { id } = req.params;
    ...
    if (error || !ledger) {
      throw new AppError('Failed to update ledger', 500);
    }
...
export const deleteLedger = async (
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from('ledgers').delete().eq('id', id);

    if (error) {
      throw new AppError('Failed to delete ledger', 500);
    }
...
export const getLedgerMembers = async (
  req: AuthenticatedRequest,
  res: Response<ApiResponse<LedgerMember[]>>
): Promise<void> => {
  try {
    const userId = req.user!.id;
    const { id } = req.params;

    const { data: userMembership, error: checkError } = await supabaseAdmin
      .from('ledger_users')
      .select('role')
      .eq('ledger_id', id)
      .eq('user_id', userId)
    ...
    if (profilesError) {
      throw new AppError('Failed to fetch member profiles', 500);
    }

    const profileMap = new Map(profiles?.map((p) => [p.id, p]) || []);
    const membersWithProfiles: LedgerMember[] = (members || []).map((m) => ({
      ...m,
...
export const inviteUser = async (
  req: AuthenticatedRequest,
  res: Response<ApiResponse<LedgerUser>>
): Promise<void> => {
  try {
    const userId = req.user!.id;
    const { id } = req.params;
