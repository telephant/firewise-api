...
import { AuthenticatedRequest, ApiResponse, Expense, ExpenseFilters } from '../types';
...
export const getExpenses = async (
  ...
  res: Response<ApiResponse<{ expenses: ExpenseWithDetails[]; total: number }>>
): Promise<void> => {
  try {
    ...
    const {
      ...
      start_date,
      end_date,
    } = req.query as unknown as ExpenseFilters & { page: string; limit: string };
    ...
    const [categories, currencies, paymentMethods, profiles] = await Promise.all([
      ...
      paymentMethodIds.length > 0
        ...
        : { data: [] },
      creatorIds.length > 0
        ? supabaseAdmin.from('profiles').select('id, full_name, email').in('id', creatorIds)
        : { data: [] },
    ]);
...
export const getExpense = async (
  req: AuthenticatedRequest,
  res: Response<ApiResponse<ExpenseWithDetails>>
): Promise<void> => {
  try {
    const userId = req.user!.id;
    ...
    const [category, currency, paymentMethod, profile] = await Promise.all([
      ...
      expense.payment_method_id
        ...
        : { data: null },
      expense.created_by
        ? supabaseAdmin.from('profiles').select('id, full_name, email').eq('id', expense.created_by).single()
        : { data: null },
    ]);
...
export const updateExpense = async (
  ...
  try {
    ...
    if (payment_method_id !== undefined) updates.payment_method_id = payment_method_id || null;
