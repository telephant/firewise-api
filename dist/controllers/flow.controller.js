...
exports.getFlowsNeedingReviewCount = exports.markFlowReviewed = exports.getFlowStats = exports.deleteFlow = exports.getFlow = exports.getFlows = void 0;
...
const getFlows = async (req, res) => {
    try {
        ...
        if (error) {
            throw new error_1.AppError('Failed to fetch transactions', 500);
        }
        ...
        (transactions || []).forEach((tx) => {
            ...
            if (tx.source_asset_id)
                assetIds.add(tx.source_asset_id);
            if (tx.debt_id)
                debtIds.add(tx.debt_id);
            if (tx.expense_category_id)
                categoryIds.add(tx.expense_category_id);
        });
        ...
        const [assetsResult, debtsResult, categoriesResult] = await Promise.all([
            ...
            debtIds.size > 0
                ? supabase_1.supabaseAdmin.from('debts').select('*').in('id', Array.from(debtIds))
                : Promise.resolve({ data: [] }),
            categoryIds.size > 0
        ...
        const debtMap = new Map((debtsResult.data || []).map((d) => [d.id, d]));
        ...
        const transactionsWithDetails = (transactions || []).map((tx) => {
            ...
            // Map transaction type to flow type for backward compatibility
            let flowType = tx.type;
            if (tx.type === 'buy')
                flowType = 'transfer';
            if (tx.type === 'sell')
                flowType = 'transfer';
            ...
            return {
                ...tx,
                // Override type with flow-compatible type (cast for backward compat)
                type: flowType,
                // New field names
                asset,
                ...
                flow_expense_category: expenseCategory,
                flow_expense_category_id: tx.expense_category_id,
                user_id: tx.belong_id, // Frontend expects user_id
                // Keep original transaction type for reference
                transaction_type: tx.type,
            };
        });
        // Add currency conversion fields if user has convert_all_to_preferred enabled
        const transactionsWithConversion = await (0, currency_conversion_1.addConvertedFieldsToArray)(transactionsWithDetails, userId);
        res.json({
            success: true,
            data: {
...
const getFlow = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        // Build query from transactions table
        let query = supabase_1.supabaseAdmin.from('transactions').select('*').eq('id', id);
        ...
        const [asset, sourceAsset, debt, expenseCategory] = await Promise.all([
            transaction.asset_id
                ? supabase_1.supabaseAdmin.from('assets').select('*').eq('id', transaction.asset_id).single()
                : { data: null },
            transaction.source_asset_id
                ? supabase_1.supabaseAdmin.from('assets').select('*').eq('id', transaction.source_asset_id).single()
                : { data: null },
            transaction.debt_id
                ? supabase_1.supabaseAdmin.from('debts').select('*').eq('id', transaction.debt_id).single()
                : { data: null },
            transaction.expense_category_id
                ? supabase_1.supabaseAdmin.from('flow_expense_categories').select('*').eq('id', transaction.expense_category_id).single()
                : { data: null },
        ]);
        // Map to from_asset/to_asset based on transaction type for backward compatibility
        ...
        const transactionWithDetails = {
            ...transaction,
            // Override type with flow-compatible type (cast to any for backward compat)
            type: flowType,
            // New field names
            asset: assetData,
            ...
            flow_expense_category_id: transaction.expense_category_id,
            user_id: transaction.belong_id,
            // Keep original transaction type for reference
            transaction_type: transaction.type,
        };
        // Add currency conversion fields if user has convert_all_to_preferred enabled
        const transactionWithConversion = await (0, currency_conversion_1.addConvertedFieldsToSingle)(transactionWithDetails, userId);
        res.json({
            success: true,
...
const deleteFlow = async (req, res) => {
    try {
        ...
        let checkQuery = supabase_1.supabaseAdmin.from('transactions').select('id').eq('id', id);
        ...
        const { error } = await supabase_1.supabaseAdmin.from('transactions').delete().eq('id', id);
        if (error) {
            throw new error_1.AppError('Failed to delete transaction', 500);
        }
...
const getFlowStats = async (req, res) => {
    try {
        ...
        if (error) {
            throw new error_1.AppError('Failed to fetch transaction stats', 500);
        }
...
const markFlowReviewed = async (req, res) => {
    try {
        ...
        const { data: transaction, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error) {
            throw new error_1.AppError('Failed to mark transaction as reviewed', 500);
        }
