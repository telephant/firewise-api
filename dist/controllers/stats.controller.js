...
const getExpenseStats = async (req, res) => {
    try {
        ...
        const { data: currencies, error: currError } = await supabase_1.supabaseAdmin
        ...
        if (!targetCurrency) {
            ...
            const { data: ledger } = await supabase_1.supabaseAdmin
                .from('ledgers')
        ...
        const { data: exchangeRates } = await supabase_1.supabaseAdmin
        ...
        let query = supabase_1.supabaseAdmin.from('expenses').select('category_id, amount, currency_id').eq('ledger_id', ledgerId);
        if (start_date)
...
const getMonthlyStats = async (req, res) => {
    try {
        const userId = req.user.id;
        const { ledgerId } = req.params;
        const { currency_id, months: monthsParam, category_id } = req.query;
